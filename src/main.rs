...
fn main() -> Result<(), Box<dyn Error>> {
  ...
  "##;
	...
	// let spans = lazy.find("span")?;
	// println!("all_spans===>:{}", spans.text());
	// let inner_spans = spans.filter("span span")?;
	// println!("innser_spans ===>{}", inner_spans.text());
	// let divs = inner_spans.parents("div")?;
	// println!("divs:{}", divs.text());
	...
	// test.add_class("haha ooo");
	// println!("{}", root.outer_html());
	// test.toggle_class("haha gugu");
	// println!("{}", root.outer_html());
	// test.remove_class("test");
	// println!("{}", root.outer_html());
	// test.remove_attr("class");
	// println!("{}", root.outer_html());
	test.eq(1)?.add_class("test abc").remove_class("test");
	let total = test.eq(0)?.set_text("    ").find(".olele")?.length();
	println!("total====>{}", total);
	println!("{}", root.outer_html());
	Ok(())
}
