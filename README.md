...
| `insertion` | When you don't need mutation the DOM, you can ignore this feature flag.             | `append(&mut x)` `append_to()` `prepend()` `prepend_to()` `insert_after()` `after()` `insert_before()` `before()` `replace_with()` | `visdom = { version = xxx, features = ["insertion"]}` |
